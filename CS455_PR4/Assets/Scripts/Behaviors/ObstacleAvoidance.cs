...
public class ObstacleAvoidance : Seek
{
	...
	bool hitcenter = Physics.Raycast(character.transform.position, ray, out hit, lookahead) && hit.rigidbody == null; //hit.transform.name != target.transform.name ;
	bool hitright = Physics.Raycast(character.transform.position, rayR, out hitR, lookaside) && hitR.rigidbody == null; //hitR.transform.name != target.transform.name;
	bool hitleft = Physics.Raycast(character.transform.position, rayL, out hitL, lookaside) && hitL.rigidbody == null; //hitL.transform.name != target.transform.name;
	...
	if ( hitcenter ){
	    ...
	    dummyTarget.GetComponent<Renderer>().enabled = false;
	    	/* If nothing to avoid, take no action */
	}
